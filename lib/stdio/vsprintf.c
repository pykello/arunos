 ...
 */
int
vsprintf(char *target, const char *format, va_list ap)
{
	int format_index = 0;
	int target_index = 0;
	...
	while (format[format_index] != 0)
	{
		...
		switch (format_flag)
		{
			...
			case 's':
			{
				...
				target_index += print_string(target + target_index, string_arg);
			...
			case 'c':
			{
				char char_arg = va_arg(ap, int);
				target[target_index] = char_arg;
				target_index++;
			...
			case 'd':
			{
				...
				target_index += print_int(target + target_index, int_arg);
...
static int
print_string(char *target, const char *str)
{
	int target_index = 0;
	int str_index = 0;
...
static int
print_int(char *target, int number)
{
	int length = 0;
	if (number < 0)
	{
...
static int
print_uint_in_base(char *target, uint32_t number, uint32_t base)
{
	int length = 0;
